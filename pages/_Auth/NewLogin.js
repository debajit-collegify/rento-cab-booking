...
class NewLogin extends Component {
    ...
    addUser = () => {
        ...
        if( Validation.stringValidate(state.signUpFromData.firstName) &&
            ...
            Validation.stringValidate(state.signUpFromData.address) ){
                ...
                axios.post('http://4eb3aff0.ngrok.io/api/auth/signup',signUpFromData).then((response) => {
                    const res = response;
                    console.log(res);
    ...
    signIn = (e) => {
        ...
        if(Validation.emailValidate(this.state.loginFormData.email) &&
            ...
            axios.post('http://4eb3aff0.ngrok.io/api/auth/signin',signInFromData).then((response) => {
                const res = response;
                console.log(res);
                // Router.pushRoute('/cab/list');
                if(res.status === 200 && res.statusText === "OK"){
